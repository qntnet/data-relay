...
import json
import logging
from secgov.conf import LAST_ID_FILE_NAME, BASE_URL, FORMS_DIR_NAME
...
def sync():
    ...
    last_id = 0
    try:
        with open(LAST_ID_FILE_NAME, 'r') as f:
            last_id = f.readline()
            last_id = last_id.strip()
            last_id = int(last_id)
    except:
        logger.exception("can't read " + LAST_ID_FILE_NAME)
    while True:
        ...
        for r in lst:
            d = r['date'].split("-")
            path = os.path.join(FORMS_DIR_NAME, r['type'].replace('/', '-'), *d, r['cik'])
