...
def get_fillings(types=None, ciks=None, facts=None, skip_segment=None, min_date=None, max_date=None, offset=0, limit=1):
    ...
    for r1 in sorted(os.listdir(SECGOV_FORMS_DIR_NAME)):
        if types is not None and r1 not in types:
            continue
        root1 = os.path.join(SECGOV_FORMS_DIR_NAME, r1)
        for r2 in sorted(os.listdir(root1)):
            root2 = os.path.join(root1, r2)
            for r3 in sorted(os.listdir(root2)):
                root3 = os.path.join(root2, r3)
                for r4 in sorted(os.listdir(root3)):
                    d = datetime.date(int(r2), int(r3), int(r4))
                    if min_date is not None and d < min_date:
                        continue
                    if max_date is not None and d > max_date:
                        continue
                    root4 = os.path.join(root3, r4)
                    for r5 in sorted(os.listdir(root4)):
                        if ciks is not None and r5 not in ciks:
                            continue
                        root5 = os.path.join(root4, r5)
                        for r6 in sorted(os.listdir(root5)):
                            ...
                            if limit is not None:
                                if limit == 0:
                                    return
                                limit -= 1
                            root6 = os.path.join(root5, r6)
    # for root6 in glob.glob(SECGOV_FORMS_DIR_NAME + '/*/*/*/*/*/*'):
    #     rp = root6.split('/')[-6:-1]
    #     if types is not None and rp[-5] not in types:
    #         continue
    ...
    #     if max_date is not None and d > max_date:
    #         continue
    #     if ciks is not None and rp[-1] not in ciks:
    #         continue


                            # for r6 in os.listdir(root5):
                            #root6 = os.path.join(root5, r6)
                            with gzip.open(os.path.join(root6, 'meta.json.gz'), 'rt') as f:
                                raw = f.read()
                            obj = json.loads(raw)
                            fact_list = []
                            obj['facts'] = fact_list
                            facts_root = os.path.join(root6, 'facts')
                            for fact_fn in sorted(os.listdir(facts_root)):
                                if fact_fn[-8:] != '.json.gz':
                                    continue
                                if f is not None and fact_fn[:-8] not in facts:
                                    continue
                                with gzip.open(os.path.join(facts_root, fact_fn), 'rt') as f:
                                    raw = f.read()
                                facts_part = json.loads(raw)
                                for i in facts_part:
                                    if skip_segment is not None and skip_segment and i.get('segment') is not None:
                                        continue
                                    fact_list.append(i)
                            yield obj
